    ...
    if '-' in word:
        for index, letter in enumerate(list(word)):
            if letter == '-':       
                blanklist[index] = '-'
    lives_lost = 0
    return word, guess_list, blanks, blanklist, lives_lost, olist     #This return statement is only here for debugging puposes          
...
def interface(guess_list, blanklist, olist):
    ...
    return f'''Letters chosen: {guesses}
    {blanks}   lives: {lives} {o}'''  #This return statement is only here for debugging puposes
